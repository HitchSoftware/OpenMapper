namespace OpenMapper.UnitTests.MappingExceptions;
...
public class When_encountering_a_member_mapping_problem_during_mapping : NonValidatingSpecBase
{
    ...
    public void Should_provide_a_contextual_exception()
    {
        ...
        typeof(OpenMapperMappingException).ShouldBeThrownBy(() => Mapper.Map<Source, Dest>(source));
    }
    ...
    {
        var source = new Source { Value = "adsf" };
        OpenMapperMappingException thrown = null;
        try
        {
            Mapper.Map<Source, Dest>(source);
        }
        catch (OpenMapperMappingException ex)
        {
            thrown = ex;
        }
