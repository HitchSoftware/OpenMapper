namespace OpenMapper.UnitTests
{
    namespace AssemblyScanning
    {
        ...
        {
            private static readonly Assembly OpenMapperAssembly = typeof(When_scanning_by_name).Assembly;

            protected override MapperConfiguration CreateConfiguration() => new(cfg =>
            {
                AppDomain.CurrentDomain.AssemblyResolve += OnAssemblyResolve;
                cfg.AddMaps(new[] { OpenMapperAssembly.FullName });
                AppDomain.CurrentDomain.AssemblyResolve -= OnAssemblyResolve;
            });

            private static Assembly OnAssemblyResolve(object sender, ResolveEventArgs args) => args.Name == OpenMapperAssembly.FullName ? OpenMapperAssembly : null;
