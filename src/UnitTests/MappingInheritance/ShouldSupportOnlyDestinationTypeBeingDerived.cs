...
public class AsWithMissingMap : NonValidatingSpecBase
{
    ...
    public void Should_report_missing_map() => new Action(AssertConfigurationIsValid).ShouldThrow<InvalidOperationException>().Message.ShouldBe(
        "Missing map from OpenMapper.UnitTests.MappingInheritance.AsWithMissingMap+TModel to OpenMapper.UnitTests.MappingInheritance.AsWithMissingMap+TConcrete. Create using CreateMap<TModel, TConcrete>.");
}
public class AsShouldWorkOnlyWithDerivedTypesWithGenerics : OpenMapperSpecBase
{
    class Source<T>
    {
    }
...
public class AsWithGenerics : OpenMapperSpecBase
{
