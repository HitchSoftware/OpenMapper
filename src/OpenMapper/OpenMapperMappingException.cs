...
/// </summary>
public class OpenMapperMappingException : Exception
{
    private readonly string _message;

    public OpenMapperMappingException()
    {
    }

    public OpenMapperMappingException(string message)
        : base(message) => _message = message;

    public OpenMapperMappingException(string message, Exception innerException)
        : base(message, innerException) => _message = message;

    public OpenMapperMappingException(string message, Exception innerException, TypePair types)
        : this(message, innerException) => Types = types;

    public OpenMapperMappingException(string message, Exception innerException, TypeMap typeMap)
        : this(message, innerException, typeMap.Types) => TypeMap = typeMap;

    public OpenMapperMappingException(string message, Exception innerException, MemberMap memberMap)
        ...
        get
        {
            return string.Join(Environment.NewLine,
                base.StackTrace
                    ...
                    .Where(str => !str.TrimStart().StartsWith("at OpenMapper.")));
        }
    }
#endif
}
...
public class OpenMapperConfigurationException : Exception
{
    ...
    public OpenMapperConfigurationException(string message)
        : base(message)
    {
    }

    public OpenMapperConfigurationException(string message, Exception inner)
        : base(message, inner)
    {
    }

    public OpenMapperConfigurationException(TypeMapConfigErrors[] errors) => Errors = errors;

    public OpenMapperConfigurationException(TypePair types) => Types = types;
    ...
    public override string Message
    {
        get
        {
            if (Types.HasValue)
            {
                ...
                while (exToUse != null)
                {
                    if (exToUse is OpenMapperConfigurationException configExc)
                    {
