...
public sealed class TypeMap
{
    ...
    public string CheckRecord() => ConstructorMap?.Ctor is ConstructorInfo ctor && ctor.IsFamily && ctor.Has<CompilerGeneratedAttribute>() ?
        " When mapping to records, consider using only public constructors. See https://docs.OpenMapper.org/en/latest/Construction.html." : null;
    ...
    public void CheckProjection()
    {
        if (Projection)
        {
            throw new OpenMapperConfigurationException("CreateProjection works with ProjectTo, not with Map.", MissingMapException(Types));
        }
    }
