...
{
    /// <summary>
    /// Dry run all configured type maps and throw <see cref="OpenMapperConfigurationException"/> for each problem
    /// </summary>
    ...
    /// Use if you want OpenMapper to compile all mappings up front instead of deferring expression compilation for each first map.
    /// </summary>
...
public sealed class MapperConfiguration : IGlobalConfiguration
{
    ...
    LambdaExpression IGlobalConfiguration.BuildExecutionPlan(in MapRequest mapRequest)
    {
        ...
        LambdaExpression GenerateObjectMapperExpression(in MapRequest mapRequest, IObjectMapper mapper)
        {
            ...
            if (mapper == null)
            {
                OpenMapperMappingException exception = new("Missing type map configuration or unsupported mapping.", null, mapRequest.RuntimeTypes)
                {
    ...
    public static OpenMapperMappingException GetMappingError(Exception innerException, in MapRequest mapRequest) =>
