    ...
    public void AssertConfigurationIsValid(TypeMap[] typeMaps)
    {
        ...
        var badTypeMaps =
            (from typeMap in typeMaps
             ...
             select new OpenMapperConfigurationException.TypeMapConfigErrors(typeMap, unmappedPropertyNames, canConstruct)).ToArray();
        if (badTypeMaps.Length > 0)
        {
            configExceptions.Add(new OpenMapperConfigurationException(badTypeMaps));
        }
        ...
        void DryRunTypeMap(TypePair types, TypeMap typeMap, MemberMap memberMap)
        {
            ...
            else
            {
                ...
                if (mapperToUse == null)
                {
                    configExceptions.Add(new OpenMapperConfigurationException(memberMap.TypeMap.Types) { MemberMap = memberMap });
                    return;
                }
